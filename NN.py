...
def load_data(slice_size):
    ...
    # Change the paths to the output directory of "dataplotting.py"

    ext = ".csv"
    path = "~\Final_Data" + str(slice_size) + ext
    path_y_hot = "~\Y_One_Hot"+ str(slice_size) + ext
    path_test_data = "~\Test_Data"+ str(slice_size) + ext
    path_y_hot_test = "~\Y_One_Hot_Test"+ str(slice_size) + ext
    path_test_time_series = "~\Test_Time_Series"+ str(slice_size) + ext
...
def main(slice_size,l):
    ...
    with tf.Session() as sess:
        ...
        fin_acc = []
        for num_of_test_samples in range(1, 50, 3):
        ...
        if(test_one_meter == True):
            #plt.ion()
            #plt.show()
            save = np.zeros([int(np.max(np.unique(test_data_meter))),50]) 
            for meter_number in np.unique(test_data_meter):
                ...
                save[int(meter_number)-1,:] = av
                #x = np.arange(1,100,2)
                #plt.plot(x,av,'r',alpha= 0.4)
                #  FOR MATLAB FIGURE, COLLECT av into array and save it as a csv file.
                ...
                #print(av)
           #path  = "C:\Users\cridn_000\Documents\KTH5\ML_Utility_Submetering\test_meter_data.csv"
            np.savetxt("test_meter_accuracy",save)
